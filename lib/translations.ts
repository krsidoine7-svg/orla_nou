export const translations = {
  fr: {
    tagline: "Simplifiez vos échanges, augmentez vos gains",
    nav: {
      home: "Accueil",
      services: "Services",
      about: "À propos",
      portfolio: "Portfolio",
      blog: "Blog",
      contact: "Contact",
      faq: "FAQ",
    },
    hero: {
      badge: "🚀 Automatisation IA Avancée",
      title: "Automatisez vos tâches répétitives",
      subtitle:
        "Libérez votre temps et boostez votre productivité avec nos solutions d'automatisation sur mesure. Basés à Abidjan, nous aidons les PME et indépendants à se concentrer sur l'essentiel.",
      slogan: "Orlan-ou — Simplifiez vos échanges, augmentez vos gains",
      stats: {
        productivity: "Productivité",
        timeSaved: "Temps économisé",
        automation: "Automatisation",
      },
    },
    services: {
      title: "Nos Services d'Automatisation",
      subtitle:
        "Des solutions complètes pour automatiser vos processus métier et optimiser votre efficacité opérationnelle.",
      items: {
        invoicing: {
          title: "Facturation Automatisée",
          description: "Automatisation complète de vos factures et paiements",
          features: [
            "Génération automatique des factures",
            "Suivi des paiements en temps réel",
            "Relances automatiques",
            "Intégration comptable",
          ],
        },
        notifications: {
          title: "Notifications & Rappels",
          description: "Système de notifications intelligentes et personnalisées",
          features: [
            "Rappels automatiques",
            "Notifications multi-canaux",
            "Alertes personnalisées",
            "Suivi des échéances",
          ],
        },
        social: {
          title: "Réseaux Sociaux",
          description: "Automatisation de votre présence sur les réseaux sociaux",
          features: [
            "Publication automatique",
            "Planification de contenu",
            "Réponses automatisées",
            "Analyse des performances",
          ],
        },
        reports: {
          title: "Rapports Automatisés",
          description: "Génération et gestion automatique de vos rapports",
          features: ["Rapports personnalisés", "Envoi automatique", "Tableaux de bord", "Analyses prédictives"],
        },
        crm: {
          title: "Intégration CRM",
          description: "Synchronisation et automatisation de votre CRM",
          features: [
            "Synchronisation des données",
            "Suivi automatique des leads",
            "Workflows personnalisés",
            "Intégrations multiples",
          ],
        },
        email: {
          title: "Tri de Mails",
          description: "Organisation intelligente de votre messagerie",
          features: ["Tri automatique", "Filtres intelligents", "Réponses automatiques", "Archivage organisé"],
        },
        custom: {
          title: "Solutions Personnalisées",
          description: "Automatisations sur mesure selon vos besoins",
          features: ["Analyse des besoins", "Développement sur mesure", "Formation incluse", "Support continu"],
        },
      },
      cta: {
        title: "Prêt à automatiser votre entreprise ?",
        description:
          "Contactez-nous pour une consultation gratuite et découvrez comment nous pouvons transformer votre productivité.",
      },
    },
    cta: {
      getStarted: "Commencer maintenant",

      contact: "Nous contacter",
      learnMore: "En savoir plus",
      getQuote: "Obtenir un devis",
      viewWork: "Voir nos réalisations",
    },
    assistant: {
      welcome: "Bonjour ! Je suis TOTO Asist, votre assistant virtuel. Comment puis-je vous aider aujourd'hui ?",
      status: "En ligne",
      placeholder: "Tapez votre message...",
      quickActions: {
        services: "Quels sont vos services ?",
        pricing: "Quels sont vos tarifs ?",
        contact: "Comment vous contacter ?",
      },
      responses: {
        services:
          "Nous proposons 7 services principaux d'automatisation : facturation, notifications, réseaux sociaux, rapports, CRM, tri de mails et solutions personnalisées. Souhaitez-vous en savoir plus sur un service en particulier ?",
        pricing:
          "Nos tarifs varient selon vos besoins spécifiques. Nous proposons des consultations gratuites pour évaluer votre projet. Voulez-vous planifier un rendez-vous ?",
        contact:
          "Vous pouvez nous contacter via le formulaire sur cette page, nous appeler ou planifier un rendez-vous directement. Nous sommes basés à Abidjan, Côte d'Ivoire.",
        about:
          "Orlan-ou est une entreprise spécialisée dans l'automatisation, fondée par Toto. Nous aidons les PME et indépendants à optimiser leurs processus métier grâce à l'IA et l'automatisation.",
        time: "Le délai dépend de la complexité de votre projet. Les automatisations simples peuvent être mises en place en quelques jours, tandis que les solutions complexes peuvent prendre quelques semaines.",
        default:
          "Je suis là pour répondre à vos questions sur nos services d'automatisation. N'hésitez pas à me demander des informations sur nos tarifs, services ou à planifier un rendez-vous !",
      },
    },
  },
  en: {
    tagline: "Simplify your exchanges, increase your gains",
    nav: {
      home: "Home",
      services: "Services",
      about: "About",
      portfolio: "Portfolio",
      blog: "Blog",
      contact: "Contact",
      faq: "FAQ",
    },
    hero: {
      badge: "🚀 Advanced AI Automation",
      title: "Automate your repetitive tasks",
      subtitle:
        "Free up your time and boost your productivity with our custom automation solutions. Based in Abidjan, we help SMEs and freelancers focus on what matters most.",
      slogan: "Orlan-ou — Simplify your exchanges, increase your gains",
      stats: {
        productivity: "Productivity",
        timeSaved: "Time Saved",
        automation: "Automation",
      },
    },
    services: {
      title: "Our Automation Services",
      subtitle: "Complete solutions to automate your business processes and optimize your operational efficiency.",
      items: {
        invoicing: {
          title: "Automated Invoicing",
          description: "Complete automation of your invoices and payments",
          features: [
            "Automatic invoice generation",
            "Real-time payment tracking",
            "Automatic reminders",
            "Accounting integration",
          ],
        },
        notifications: {
          title: "Notifications & Reminders",
          description: "Intelligent and personalized notification system",
          features: ["Automatic reminders", "Multi-channel notifications", "Custom alerts", "Deadline tracking"],
        },
        social: {
          title: "Social Media",
          description: "Automation of your social media presence",
          features: ["Automatic posting", "Content scheduling", "Automated responses", "Performance analysis"],
        },
        reports: {
          title: "Automated Reports",
          description: "Automatic generation and management of your reports",
          features: ["Custom reports", "Automatic sending", "Dashboards", "Predictive analytics"],
        },
        crm: {
          title: "CRM Integration",
          description: "Synchronization and automation of your CRM",
          features: ["Data synchronization", "Automatic lead tracking", "Custom workflows", "Multiple integrations"],
        },
        email: {
          title: "Email Sorting",
          description: "Intelligent organization of your messaging",
          features: ["Automatic sorting", "Smart filters", "Automatic responses", "Organized archiving"],
        },
        custom: {
          title: "Custom Solutions",
          description: "Tailor-made automations according to your needs",
          features: ["Needs analysis", "Custom development", "Training included", "Ongoing support"],
        },
      },
      cta: {
        title: "Ready to automate your business?",
        description: "Contact us for a free consultation and discover how we can transform your productivity.",
      },
    },
    cta: {
      getStarted: "Get Started Now",

      contact: "Contact Us",
      learnMore: "Learn More",
      getQuote: "Get Quote",
      viewWork: "View Our Work",
    },
    assistant: {
      welcome: "Hello! I'm TOTO Asist, your virtual assistant. How can I help you today?",
      status: "Online",
      placeholder: "Type your message...",
      quickActions: {
        services: "What are your services?",
        pricing: "What are your rates?",
        contact: "How to contact you?",
      },
      responses: {
        services:
          "We offer 7 main automation services: invoicing, notifications, social media, reports, CRM, email sorting, and custom solutions. Would you like to know more about a particular service ?",
        pricing:
          "Our rates vary according to your specific needs. We offer free consultations to evaluate your project. Would you like to schedule an appointment?",
        contact:
          "You can contact us via the form on this page, call us, or schedule an appointment directly. We are based in Abidjan, Côte d'Ivoire.",
        about:
          "Orlan-ou is a company specialized in automation, founded by Toto. We help SMEs and freelancers optimize their business processes through AI and automation.",
        time: "The timeline depends on the complexity of your project. Simple automations can be implemented in a few days, while complex solutions may take a few weeks.",
        default:
          "I'm here to answer your questions about our automation services. Feel free to ask me about our rates, services, or to schedule an appointment!",
      },
    },
  },
}
